...
public interface TDReceiptsRepository  extends JpaRepository<TDReceiptsEntity, Long> {
	...
	@Modifying
    @Query(value = "UPDATE md020004 SET mprdacctid=?2, missynctocoresys=1, mmatval=?3, mreceiptstatus=?4 , merrormessage = '' WHERE mrefno = ?1  ",nativeQuery = true)
    int updateTDReceipts( int mCustRefNo, String mprdacctid, double mmatval, int mreceiptstatus );
