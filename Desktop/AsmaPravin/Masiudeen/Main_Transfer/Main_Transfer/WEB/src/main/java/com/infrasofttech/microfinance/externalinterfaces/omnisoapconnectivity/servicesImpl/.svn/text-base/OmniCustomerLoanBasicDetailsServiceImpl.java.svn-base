...
/*		CustomerLoanEntity customerLoanEntityBean = (CustomerLoanEntity) bean;
	...
	public OmniSoapResultBean omniSoapServiceswithLoanAndCustomer(int customerNo,LocalDateTime lastOpenDate,BaseEntity customerLoan ) {
		 ...
		 * retBean.setTRefno(customerLoanEntity.getTrefno()); retBean.setStatus(1);
		 * retBean.setMCustNo(0); retBean.setError("Data Not Correctly Submitted");
		 * return retBean; }
		 */
		...
		try 
		{
			...
			outparam = omniServiceSoapProxy.invoke(Constants.Field1, "1", 910920, inParams);
			...
			if(!outparam.getField201().equals("0")){
				...
				System.out.println("Coming inside first");
				retBean.setStatus(1);
			...
			if(outparam!=null && outparam.getField1() !=null) {
				System.out.println("Coming inside it" + outparam.getField1());
				...
				System.out.println("CFoming in else");
				retBean.setStatus(1);
				retBean.setLoanLeadIdOfCore("");
				retBean.setMleadsid("");
				retBean.setError(outparam.getField203());
	...
	public TMsgInParam prepareRequestToOmni(BaseEntity bean) {
		...
		if(customerLoanEntityBean.getMleadsid()==null||
			...
			if(customerLoanEntityBean.getMinststrtdt()!=null) {
