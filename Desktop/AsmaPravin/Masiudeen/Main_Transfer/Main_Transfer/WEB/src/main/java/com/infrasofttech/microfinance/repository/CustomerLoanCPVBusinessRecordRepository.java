...
public interface CustomerLoanCPVBusinessRecordRepository extends JpaRepository<CustomerLoanCPVBusinessRecordEntity, Long> {
	...
	@Modifying
    @Query(value="SELECT * FROM  md045801 where missynctocoresys =?1 AND mloanmrefno != 0",nativeQuery = true)
	...
	@Modifying
	@Query(value = "UPDATE md045801 SET mleadsid= ?1 , mlastupdatedt = ?4 WHERE mloanmrefno = ?2 AND mloantrefno = ?3  ",nativeQuery = true)
