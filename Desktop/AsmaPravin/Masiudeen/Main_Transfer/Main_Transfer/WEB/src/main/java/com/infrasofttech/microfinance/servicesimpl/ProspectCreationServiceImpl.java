		...
		try { 
			...
			for(UserMasterEntity userEntity:userList) {	 	
				...
				if(lastSynsTime==null && userList.size()==1) {
					prospectList = prospectCreationRepository.getProspectFromCreatedBy(userEntity.getMusrcode());
				
				}else if(lastSynsTime==null && userList.size()>1) {
					prospectList = prospectCreationRepository.getprspctfrmcrtdByAndprspctstatus(userEntity.getMusrcode(),4);
				}else if(lastSynsTime!=null && userList.size()==1) {
					prospectList = prospectCreationRepository.getProspectFromCreatedByAndLastSyncedDate(userEntity.getMusrcode(),lastSynsTime);
				
				}else if(lastSynsTime!=null && userList.size()>1) {
					prospectList = prospectCreationRepository.getprspctfrmcrtdByAndprspctstatusAndLastSyncedDate(userEntity.getMusrcode(),4,lastSynsTime);
				}
