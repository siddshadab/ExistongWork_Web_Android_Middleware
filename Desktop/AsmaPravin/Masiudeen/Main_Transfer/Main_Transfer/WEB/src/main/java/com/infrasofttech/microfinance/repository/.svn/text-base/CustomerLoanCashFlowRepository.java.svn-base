...
public interface CustomerLoanCashFlowRepository extends JpaRepository<CustomerLoanCashFlowEntity, Long> {
    ...
    int updateErrorFromOmni( int mrefno, String merrormesssage,int misSyncToCoreSys);
	  ...
	  @Query(value="SELECT * FROM  md045806 where mcreatedby=?1 AND mlastupdatedt > ?2",nativeQuery = true)
	  List<CustomerLoanCashFlowEntity>findByCreatedbyAndDateTime(String mcreatedby,LocalDateTime lastsynsdate);
	  
	
	  @Query(value="SELECT * FROM  md045806 where mcreatedby=?1",nativeQuery = true) 
	  List<CustomerLoanCashFlowEntity>findByCreatedby(String mcreatedby);
	  ...
	  @Modifying
	  @Query(value = "UPDATE md045806 SET mleadsid= ?1 , mlastupdatedt = ?4 WHERE mloanmrefno = ?2 AND mloantrefno = ?3  ",nativeQuery = true)
