...
public class OmniDisbursedListAuthorizationServiceImpl implements OmniDisbursedListAuthorizationService{
	...
	public List<DisbursedListEntity> omniSoapServicesDisbursedList(List<DisbursedListEntity> beanList) {
	...
	public TMsgInParam prepareRequestToOmni(BaseEntity bean) {
		...
		for(int inparamsCount =0;inparamsCount<this.beanList.size();inparamsCount++) {
			...
			temp.append(this.beanList.get(inparamsCount).getMrebateamtflag()).append("~").append(mode).append("~");
			...
			try {
		      ...
		      f1=null;
		        	for(int arrOfStringSingles =0;arrOfStringSingles<=arrOfStr.length;arrOfStringSingles++) {
		        		disbBean.setMrefno(Integer.valueOf(arrOfStr[0]));
		        		disbBean.setMissynctocoresys(Integer.valueOf(arrOfStr[1].toString())==1?0:1);
		        	disbBean.setMerrormessage(arrOfStr[2]);
