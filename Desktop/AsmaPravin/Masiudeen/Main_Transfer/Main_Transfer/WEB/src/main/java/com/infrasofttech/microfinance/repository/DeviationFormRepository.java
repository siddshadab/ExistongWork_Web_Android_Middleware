...
public interface DeviationFormRepository extends JpaRepository<DeviationFormEntity, Long> {
	...
	@Modifying
    @Query(value="SELECT * FROM  md045805 where missynctocoresys =?1   AND mloanmrefno != 0 ",nativeQuery = true)
	...
	@Modifying
	@Query(value = "UPDATE md045805 SET mleadsid= ?1 , mlastupdatedt = ?4 WHERE mloanmrefno = ?2 AND mloantrefno = ?3  ",nativeQuery = true)
