	...
	public TMsgInParam prepareRequestToOmni(BaseEntity bean) {
		...
		if(beanList!=null && beanList.size()>0){
			...
			for( inparamsCount =0;inparamsCount<this.beanList.size();inparamsCount++) {	
				...
				mLoanMrefno=beanList.get(inparamsCount).getMloanmrefno();
				temp="";	
				
				temp=temp+(beanList.get(inparamsCount).getMapplicanttype())+Constants.TILDA+beanList.get(inparamsCount).getMexistingcustyn()+Constants.TILDA+
