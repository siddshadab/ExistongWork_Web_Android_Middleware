...
@Component({
  selector: 'app-country-master-view',
  templateUrl: './country-master-view.component.html',
  styleUrls: ['./country-master-view.component.scss']
})
export class CountryMasterViewComponent implements OnInit {
  ...
  dataNavigate(list){
    ...
    if(this.rightsData.browse == 1){
      this.router.navigate(['/country-master-ind-view'], { skipLocationChange: true, queryParams: list });
    }
  }
    ...
    this.countryListArray.forEach(item => 
      {
        ...
        if(checked){
          //this.allCheckedList = [];
          this.allCheckedList.push(String(item.mcountryid));      
        }else{
          this.allCheckedList = [];
  ...
  toggleVisibility(option,e){
    ...
    if(e.target.checked) {
      ...
      this.allCheckedList.push(String(option.mcountryid));
      ...
      for(var i=0 ; i < this.allCheckedList.length; i++) {
        if(this.allCheckedList[i] == option.mcountryid) {
  ...
  multipleDeleteCheckboxes(){
    if(confirm("Are you sure to delete ?")) {
      let objBrCode = {mcountryid: this.allCheckedList};
