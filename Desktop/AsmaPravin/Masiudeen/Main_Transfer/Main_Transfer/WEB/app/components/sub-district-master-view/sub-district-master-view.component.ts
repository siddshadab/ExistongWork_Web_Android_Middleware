...
@Component({
  selector: 'app-sub-district-master-view',
  templateUrl: './sub-district-master-view.component.html',
  styleUrls: ['./sub-district-master-view.component.scss']
})
export class SubDistrictMasterViewComponent implements OnInit {

  subDistrictListArray = [];
  rightsData:any;
  sub:any;
  alert:boolean = false;
  alertMessage: any;
  ...
  ngOnInit() {
    ...
    this.apiservice.getAllSubDistricts().subscribe(res =>{
      //console.log(res);

      this.subDistrictListArray = res;
  ...
  dataNavigate(list){
    ...
    if(this.rightsData.browse == 1){
      this.router.navigate(['/sub-district-master-ind-view'], { skipLocationChange: true, queryParams: list });
    }
  }
  ...
  allCheckboxes(event) {
    ...
    this.subDistrictListArray.forEach(item => 
      {
        item.selected = checked;
        if(checked){
          //this.allCheckedList = [];
          this.allCheckedList.push(item.mplacecd);      
        }else{
          this.allCheckedList = [];
  ...
  toggleVisibility(option,e){
    ...
    if(e.target.checked) {
      ...
      this.allCheckedList.push(option.mplacecd);
      ...
      for(var i=0 ; i < this.allCheckedList.length; i++) {
        if(this.allCheckedList[i] == option.mplacecd) {
  ...
  multipleDeleteCheckboxes(){
    if(confirm("Are you sure to delete ?")) {
      let objBrCode = {mplacecd: this.allCheckedList};
