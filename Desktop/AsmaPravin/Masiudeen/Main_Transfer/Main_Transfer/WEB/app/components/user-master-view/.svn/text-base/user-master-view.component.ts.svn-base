...
@Component({
  ...
  styleUrls: ['./user-master-view.component.scss']
})
export class UserMasterViewComponent implements OnInit {
  ...
  dataNavigate(list){
    if(this.rightsData.browse == 1){
      this.router.navigate(['/user-master-ind-view'], { skipLocationChange: true, queryParams: list });
    }
  ...
  multipleDeleteCheckboxes(){
    if(confirm("Are you sure to delete ?")) {
      ...
      this.apiservice.deleteUserData(objCode).subscribe(res => {
        //console.log(res);
        if(res.merror == 200){
          destroydtSample();
          //alert(res.merrormsg);
