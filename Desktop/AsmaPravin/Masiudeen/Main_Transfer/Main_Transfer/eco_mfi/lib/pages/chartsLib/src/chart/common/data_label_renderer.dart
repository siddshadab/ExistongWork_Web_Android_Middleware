...
void _calculateDataLabelPosition(XyDataSeries<dynamic, dynamic> series,
  ...
  if (point != null && point.isVisible && point.isGap != true) {
    ...
    if (label.isNotEmpty) {
      ...
      _calculateDataLabelRegion(point, dataLabel, chart, chartLocation,
          chartLocation2, isRangeSeries, clipRect, textSize, textSize2);
    }
  }
}
...
void _drawDataLabel(
  ...
  if (label != null &&
    ...
    if (label.isNotEmpty && isDatalabelCollide
      ...
      _drawDataLabelPath(
          canvas, series, index, dataLabel, point, textStyle, opacity, label);
...
Rect _validateRect(Rect rect, Rect clipRect) {
  ...
  top = rect.top < clipRect.top ? clipRect.top + padding : rect.top;
  left -= ((left + rect.width) > clipRect.right)
      ? (left + rect.width) - clipRect.right
      : 0;
