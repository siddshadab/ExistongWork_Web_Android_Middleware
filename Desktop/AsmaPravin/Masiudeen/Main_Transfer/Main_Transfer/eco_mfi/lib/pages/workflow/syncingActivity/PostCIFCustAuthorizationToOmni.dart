...
class PostCIFCustAuthorizationToOmni{
  String timestamp() => DateTime.now().millisecondsSinceEpoch.toString();
  static Utility obj = new Utility();
  static final _headers = {'Content-Type': 'application/json'};
  var urlGetCIFDetails =
      "CIF/postCIFCustAuthorizationToOmni/";
  static const JsonCodec JSON = const JsonCodec();
  Future<List<CIFBean> > trySave(int mcustno ) async {
    bool isNetworkAvailable;
    isNetworkAvailable = await obj.checkIfIsconnectedToNetwork();
    if (isNetworkAvailable) {
      return await getMiddleWareData(mcustno  ,urlGetCIFDetails);
    }
  }
  Future<List<CIFBean> > getMiddleWareData(int mcustno, String url) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    String usrcode = prefs.get(TablesColumnFile.musrcode);
//    int lbrcode = prefs.getInt(TablesColumnFile.musrbrcode);
    String json;

    json =  await getJson(mcustno,usrcode);
  ...
  Future<String> getJson (int custno,String usrcode) async{
    Map map = new Map();
