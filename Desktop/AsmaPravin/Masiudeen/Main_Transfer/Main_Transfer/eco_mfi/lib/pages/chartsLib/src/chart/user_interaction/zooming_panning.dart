...
//Below method is for pinch zooming
  void _performPinchZooming(
    ...
    if (!(_zoomingRect.width > 0 && _zoomingRect.height > 0)) {
      ...
      final Rect elementOffsetRect = _chart._chartState.containerRect;
  ...
  void _calculatePinchZoomFactor(SfCartesianChart chart, Rect pinchRect) {
    ...
    final List<_ZoomAxisRange> _zoomAxes = chart._chartState.zoomAxes;
    for (int axisIndex = 0;
  ...
  void _calculateZoomAxesRange(SfCartesianChart chart) {
    for (int index = 0;
      ...
      if (_chart._chartState.zoomAxes != null &&
          _chart._chartState.zoomAxes.isNotEmpty &&
          index <= _chart._chartState.zoomAxes.length - 1) {
        if (!_delayRedraw) {
          _chart._chartState.zoomAxes[index].min = axisRange.minimum;
          _chart._chartState.zoomAxes[index].delta = axisRange.delta;
        }
      } else {
        ...
        _chart._chartState.zoomAxes.add(range);
      }
    }
  }
