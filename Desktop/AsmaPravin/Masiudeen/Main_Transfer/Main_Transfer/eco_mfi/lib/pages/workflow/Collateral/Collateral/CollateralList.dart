...
class CollateralList extends StatefulWidget {
  ...
  final int mrefno;
  final int trefno;
  CollateralList(this.mleadsId, this.mrefno, this.trefno);
  @override
...
class _CollateralList extends State<CollateralList> {
  ...
  List<CollateralBasicSelectionBean> items =
              ...
              Container(
                ...
                child: Container(
                  ...
                  child: Column(
                    children: <Widget>[
                      new Text(
                        " ${items[index].collatrlTyp == "33" ? "Vehicle" : "Land and property"}",
...
//mainAxisAlignment: MainAxisAlignment.spaceAround,
     ...
     globals.sessionTimeOut.SessionTimedOut();
                                        _AddCollateraBasedOnTyp(items[index]);
                                      },
    ...
    if (count == 1 || count == 2) {
      ...
      loanDetailsBuilder = new FutureBuilder(
          future: AppDatabase.get()
              .getCollatrlDetailsList(widget.trefno, widget.mrefno)
              .then((List<CollateralBasicSelectionBean> gaurantorData) {
            items.clear();
            storedItems.clear();
      ...
      loanDetailsBuilder = ListView.builder(
          ...
          itemBuilder: (context, position) {
            ...
            return new GestureDetector(
              behavior: HitTestBehavior.opaque,
              onTap: () {
                _onTapItem(items[position], widget.mleadsId, widget.mrefno,
                    widget.trefno);
              },
              child: new Card(
                //color Color(0xff2f1f4),
                      ...
                      Container(
                        ...
                        child: Container(
                          ...
                          child: Column(
                            children: <Widget>[
                              new Text(
                                " ${items[position].collatrlTyp == "33" ? "Vehicle" : "Land and property"}",
                                overflow: TextOverflow.ellipsis,
                                style: new TextStyle(
                                  fontSize: 18.0,
                                  color: Colors.white,
     ...
     globals.sessionTimeOut.SessionTimedOut();
              setState(() {
                if (this.actionIcon.icon == Icons.search) {
                  ...
                  this.appBarTitle = new Text("Savings List" + svngListLeng);
      ...
      floatingActionButton: new FloatingActionButton(
          ...
          backgroundColor: Colors.green,
           onPressed: () { globals.sessionTimeOut=new SessionTimeOut(context: context);
     globals.sessionTimeOut.SessionTimedOut();
            _addNewCollateral(widget.mleadsId, widget.mrefno, widget.trefno);
          }),
      body: loanDetailsBuilder,
    );
  }

  void _addNewCollateral(String mleadsId, int mrefno, int trefno) {
    Navigator.push(
      context,
      new MaterialPageRoute(
          builder: (context) => new AddCollaterals(
              mleadsId: mleadsId,
              mrefno: mrefno,
              trefno: trefno)), //When Authorized Navigate to the next screen
    );
  }

  void _onTapItem(CollateralBasicSelectionBean item, String mleadsId,
      int mrefno, int trefno) {
  ...
  void filterList(String val) async {
    ...
    storedItems.forEach((obj) {
      if (obj.mfname.toString().contains(val) ||
          ...
          obj.mcustno.toString().contains(val)) {
        items.add(obj);
      }
  ...
  Future<Null> _AddCollateraBasedOnTyp(
      CollateralBasicSelectionBean collateralBasicSelectionBean) async {
    print("collateralBasicSelectionBean.collatrlTyp" +
  ...
  Future deleteCollateralDetails(CollateralBasicSelectionBean item) async {
    String delBasicTableColl =
        "Collaterals_Master where mrefno = ${item.mrefno}  And Trefno = ${item.trefno}";
    String delRemTableColl =
        "Collateral_REM_land_and_house_Master where colleteralmrefno = ${item.mrefno}  And colleteraltrefno = ${item.trefno}";
    String delVehTableColl =
        "Collateral_Vehicle_Master where colleteralmrefno = ${item.mrefno}  And colleteraltrefno = ${item.trefno}";
  ...
  Future<bool> callDialog(CollateralBasicSelectionBean BEAN) {
    onPop(
        context, 'Are you sure?', 'Do you want to Go Delete this record', BEAN);
  }
