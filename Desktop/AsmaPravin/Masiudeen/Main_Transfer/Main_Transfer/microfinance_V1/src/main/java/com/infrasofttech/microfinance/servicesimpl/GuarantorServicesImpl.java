...
//	}
	...
	@Override
		...
		try {
			List<GuarantorEntity> guarantorListEntity =new ArrayList<GuarantorEntity>();
            for(GuarantorEntity bean: group) {
            	//Check in mrefno and trefno
            	
            	
            	if(bean.getMloanmrefno()!=0 &&bean.getTrefno()!=0) {
            		
            		GuarantorEntity ent = grntrrepo.findBymloanmrefnoandtrefno(bean.getMloanmrefno(),bean.getTrefno());
            		if(ent==null) {
            			bean.setMissynctocoresys(0);
            			guarantorListEntity.add(bean);
            			
            		}else {
            			
            			bean.setMrefno(ent.getMrefno());
            			bean.setMissynctocoresys(ent.getMissynctocoresys());
            			guarantorListEntity.add(bean);
            		}
...
//                if( bean.getMrefno()!=0) {
//                    System.out.println("getting guarantor mrefno"+bean.getMrefno());
//                   
//                    if(bean.getMissynctocoresys()==1) {
//                        int  isSynctoCoresys = grntrrepo.findByMrefNo(bean.getMrefno());
//                        if(isSynctoCoresys==0) {
//                            bean.setMissynctocoresys(0);   
//                        }
//                       
//                       
...
//                }
			...
			return new ResponseEntity<Object>(grntrrepo.saveAll(guarantorListEntity),new HttpHeaders(),HttpStatus.CREATED);
		} catch (Exception e) {
	...
	public void updateErrorfromMrefTref(String merrormessage, int mloanmrefno, int mloantrefno) {
		try {
			grntrrepo.updateErrorFromLoanMrefTref(merrormessage, mloanmrefno, mloantrefno,LocalDateTime.now());
