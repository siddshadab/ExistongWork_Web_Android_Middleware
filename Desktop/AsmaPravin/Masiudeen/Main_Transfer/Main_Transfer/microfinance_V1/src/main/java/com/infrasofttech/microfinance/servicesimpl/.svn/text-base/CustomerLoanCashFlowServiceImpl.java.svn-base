...
try {	 
	...
	for(CustomerLoanCashFlowEntity bean: cashFlowEntity) {
		...
		if( bean.getMrefno()!=0) {
			...
			if(cashFlowBean!=null) {
				if(bean.getMlastupdatedt().withNano(0).isAfter(cashFlowBean.getMlastupdatedt().withNano(0)) ) {
					if(bean.getMissynctocoresys()==0) {//if Updated then only
	...
	public ResponseEntity<?> findByCreatedByAndLastSyncedTime(CustomerLoanCashFlowEntity cashFlowEntity) {
		// TODO Auto-generated method stub
		 try {
			...
			return ResponseEntity.notFound().build(); 
			  return  new ResponseEntity<List<CustomerLoanCashFlowEntity>>(new HttpHeaders(),HttpStatus.OK); }catch(Exception e) {
	...
	public void updateErrorfromMrefTref(String merrormessage, int mloanmrefno, int mloantrefno) {
		try {
			custLoanCashFlowRepo.updateErrorFromLoanMrefTref(merrormessage, mloanmrefno, mloantrefno,LocalDateTime.now());
