...
public interface CentersFoundationsRepository extends JpaRepository<CentersFoundationEntity, Long> { 
	...
	List<CentersFoundationEntity> findByMcenternameAndMrefno(String mcentername,int mrefno);
	 ...
	 CentersFoundationEntity findByTrefAndMcreatedByAndIsSynced(long centerNumberOfTab,String usrCode , int mrefno,int isdataSynced);
		//findBymcreated

	
	 @Query(value = "Select * from  md001006  WHERE mcreatedby = ?1 AND mgeolatd IS NOT NULL AND mgeologd IS NOT NULL ",nativeQuery = true)
		List<CentersFoundationEntity> findByMcreatedByLOcationNotNull(String mcreatedby);
	 
	 @Modifying
	 	@Query(value= "UPDATE MD001006 SET mgeolatd = ?1 , mgeologd = ?2 WHERE mrefno = ?3 ", nativeQuery = true)
	 	int updateCenterLocationFrmMrefno(String mgeolatd, String mgeologd, int mrefno);
