...
public interface KycMasterRepository extends JpaRepository<KycMasterEntity , Long> {
	...
	@Query(value="SELECT TOP 50 * FROM  md045804 WITH (NOLOCK) where missynctocoresys =?1 AND mloanmrefno != 0 ",nativeQuery = true)
    ...
    int updateErrorStatus( int mrefno, String merrormessage);
	 ...
	 @Modifying
	  @Query(value = "UPDATE md045804 SET mleadsid= ?1, missynctocoresys = 0 , mlastupdatedt = ?4 , merrormessage =''  WHERE mloanmrefno = ?2 AND mloantrefno = ?3  ",nativeQuery = true)
	 ...
	 int updateStatusAndLeadid( int mrefno,String mleadsId);     
	  ...
	  @Modifying
	  @Query(value = "UPDATE md045804 SET merrormessage= ?1 , missynctocoresys = 2 ,mlastupdatedt = ?4 WHERE mloanmrefno = ?2 AND mloantrefno = ?3  ",nativeQuery = true)
