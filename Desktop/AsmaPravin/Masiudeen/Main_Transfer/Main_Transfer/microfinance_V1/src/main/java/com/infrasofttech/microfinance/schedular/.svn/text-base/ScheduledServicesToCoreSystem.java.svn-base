...
public class ScheduledServicesToCoreSystem {
	...
	@Autowired
	LicenseMasterServiceImpl licenceMasterServiceImpl;
...
//		System.out.println("session variable final 201 "+ Constants.Field1);
		if("1".equals(isSceduleReq)) {
			if(Constants.Field204 != null && Constants.Field204.equals("") ) {
				...
				Constants.OMNIUSER = envr.getProperty("OMNIUSER");
				Constants.OMNIPASS = envr.getProperty("OMNIPASS");
...
/*	@Scheduled(fixedDelay = 30000)
	...
	public   void LoadLicenseValidation() {
		licenceMasterServiceImpl.updateLiceseValidation();
	}	
	
	
	
	@Scheduled(fixedDelay = 300000)
	public void mergeInternalBankTransfer() {
		 String isSceduleReq =envr.getProperty("internalbanktranferschedule"); 
		 if("1".equals(isSceduleReq)) {
				if(Constants.Field204 != null && !Constants.Field204.equals("") ) {
					//System.out.println("if k andr aa hai..");
					syncronizeIntrenalBankTransfer();
				}
			...
			for (CustomerEntity objEntity : listEntity) {
				...
				try {
					...
					savingsListServicesImpl.updateSavingsCustNo(objEntity.getMrefno(), bean.getMCustNo());	
						...
						objEntity.setMretry(objEntity.getMretry()+1);
						objEntity.setMerrormessage("Response Blank Try again");
					...
					if(!(custno==0)) {
						...
						try {
							...
							if(bean==null) {
								...
								//objEntity.setMerrormessage("System Busy Try again");
								//savingsListServicesImpl.updateSavingsErrorfromOmniWithouLastSynsDate(objEntity.getMrefno(),objEntity.getMerrormessage());
								
							}
							
							else if (bean != null && bean.getStatus() == 0) {
								...
								savingsListServicesImpl.updateSavingsList(bean.getMCustRefNo(), bean.getMprdacctid(),
										bean.getMcrs(),LocalDateTime.now());
								...
								if(objEntity.getMretry()<3) {
									System.out.println("Galat retyr k nadar");
									savingsListServicesImpl.updateSavingsErrorfromOmni(bean.getMCustRefNo(), bean.getError(),objEntity.getMretry()+1);
									
								}
								else {
									System.out.println("mref no hai "+ bean.getMCustRefNo());
									System.out.println("error hai "+ bean.getError());
									System.out.println("DateTiem hai "+ LocalDateTime.now());
									savingsListServicesImpl.updateSavingsErrorfromOmni(bean.getMCustRefNo(), bean.getError(),LocalDateTime.now());
		...
		if (bean != null & bean.getStatus() == 0) {
			...
			loanServiceImpl.updateErrorfromOmni(customerLoanEntity.getMrefno(), bean.getError());
				...
				for(LoanLevelMasterEntity loanLevelEntity :loanLevelList) {
					if(loanLevelEntity.getMbuttonid()==8) {
						try {
							cashFlowServiceImpl.updateErrorfromMrefTref(Constants.MissingLeadError,customerLoanEntity.getMrefno(),
									customerLoanEntity.getTrefno());
						}catch(Exception e) {	
							System.out.println("Error in updating LeadId of Cashflow");
						}
						...
						try {
							kycMasterServiceImpl.updateErrorfromMrefTref(Constants.MissingLeadError,customerLoanEntity.getMrefno(),
									customerLoanEntity.getTrefno());
						}catch(Exception e) {	
							System.out.println("Error in updating LeadId of KYC");
						}
						...
						try {
							guarantorServicesImpl.updateErrorfromMrefTref(Constants.MissingLeadError,customerLoanEntity.getMrefno(),
									customerLoanEntity.getTrefno());
						}catch(Exception e) {	
							System.out.println("Error in updating LeadId of Guarantor");
						}					
						...
						try {
							cpvBusinessRecordServiceImpl.updateErrorfromMrefTref(Constants.MissingLeadError,customerLoanEntity.getMrefno(),
									customerLoanEntity.getTrefno());
						}catch(Exception e) {	
							System.out.println("Error in updating LeadId of CPV");
						}
						...
						try {
							socialAndEnvironmentalServiceImpl.updateErrorfromMrefTref(Constants.MissingLeadError,customerLoanEntity.getMrefno(),
									customerLoanEntity.getTrefno());
						}catch(Exception e) {	
							System.out.println("Error in updating LeadId of Social");
						}
						...
						try {
							tradeAndNeighbourRefCheckServiceImpl.updateErrorfromMrefTref(Constants.MissingLeadError,customerLoanEntity.getMrefno(),
									customerLoanEntity.getTrefno());
						}catch(Exception e) {	
							System.out.println("Error in updating LeadId of Trade");
						}
						...
						try {
							deviationFormServiceImpl.updateErrorfromMrefTref(Constants.MissingLeadError,customerLoanEntity.getMrefno(),
									customerLoanEntity.getTrefno());
						}catch(Exception e) {	
							System.out.println("Error in updating LeadId of Deviation");
						}
			...
			for (TDReceiptsEntity objEntity : tDReceiptsEntity) {
				...
				if (bean != null & bean.getStatus() == 0) {
					...
					tDReceiptsServicesImpl.updateTDReceipts(bean.getMCustRefNo(), bean.getMprdacctid(),
							bean.getMmatval(), bean.getMreceiptstatus(),bean.getMbatchcd(),bean.getMsetno(),bean.getMscrollno());
	...
	public void synchronizeDailyLoanCollectionToOmni() {
		...
		List<DailyLoanCollectedEntity> finalListEntity = new ArrayList<DailyLoanCollectedEntity>();
		
		if (listEntity != null && listEntity.size() > 0) {
			for(int i = 0;i<listEntity.size();i++) {
				
				
				List<DailyLoanCollectedEntity> dedupCheckList  = dailyLoanCollectedImpl.getDuplicateLoanCollections(listEntity.get(i).getMprdacctid(), listEntity.get(i).getMcreateddt());
				
				if(dedupCheckList!=null&&dedupCheckList.size()==1) {
					finalListEntity.add(listEntity.get(i));
				}
				else if(dedupCheckList!=null&&dedupCheckList.size()>1) {
					//CHecking either it already exists or every dedup is new 
					
					boolean  alreadySynced = false;
					int alreadySyncedmrefno = 0;
					for(DailyLoanCollectedEntity  loanCollected:  dedupCheckList) {
						
						
						if(loanCollected.getMissynctocoresys()==1||
								(loanCollected.getMerrormessage()!=null&&(!loanCollected.getMerrormessage().trim().equals("")) )
								
								) {
							
							alreadySynced = true;
							alreadySyncedmrefno = loanCollected.getMrefno();
							break;
					...
					if(alreadySynced==true) {
						...
						//if already synced updating the remailing One
						dailyLoanCollectedImpl.dedupCheck(listEntity.get(i),alreadySyncedmrefno,Constants.ALREADYUPDATEERROR);
						
					}
					else {
						
						//Not already Synced updating other then this one
						listEntity.get(i).setMerrormessage(Constants.DEDUPCHECKERROR);
						dailyLoanCollectedImpl.dedupCheck(listEntity.get(i));
						finalListEntity.add(listEntity.get(i));
						 //Update DOING THIS ONE 
					}
...
//				if(listEntity.get(i).getMissynctocoresys()==1) { 
//					listEntity.get(i).setMerrormessage(Constants.ALREADYUPDATEERROR);
//					dailyLoanCollectedImpl.dedupCheck(listEntity.get(i));
//									
//					continue;
//				}
...
//				}
			...
			System.out.println("Final  list Size hai bhaioye " + finalListEntity.size());	
			if(finalListEntity!=null&& finalListEntity.size()>0) {
				List<DailyLoanCollectedEntity> listBean = omniDailyLoanCollectionServiceImpl
						.omniSoapServicesDailyCollectedList(finalListEntity);
		...
		if (groupsFoundationEntity != null) {
			...
			for (GroupsFoundationEntity objEntity : groupsFoundationEntity) {
				...
				if(objEntity.getMcenterid()==0) {
					if(objEntity.getMrefcenterid()!=0) {
						...
						if(center!=null) {
							...
							if (bean != null & bean.getStatus() == 0) {
								groupFoundationServicesImpl.updateGroupDeatilsWithCenterID(bean.getMRefno(), bean.getGroupId(),objEntity.getMcenterid());
...
//	}
				public void synchronizeSingleTDReceiptsToOmni(TDReceiptsEntity tdEntity) {
					System.out.println("synchronize Single TD Receipts To Omni");
							...
							if (bean != null & bean.getStatus() == 0) {
								...
								tdEntity.setMbatchcd(bean.getMbatchcd());
								tdEntity.setMsetno(bean.getMsetno());
								tdEntity.setMscrollno(bean.getMscrollno());
				...
				public void synchronizeSocialAndEnvToOmni() {
					...
					if (listEntity != null) {
						...
						for (SocialAndEnvironmentalEntity objEntity : listEntity) {
							i++;
							if(objEntity.getMleadsid()==null||("").equals(objEntity.getMleadsid())||(Constants.NULL).equals(objEntity.getMleadsid().trim()) ) {
								...
								if(customerLoanEntity!=null&&customerLoanEntity.getMleadsid()!=null&&!("").equals(customerLoanEntity.getMleadsid())) {
									...
									socialAndEnvironmentalServiceImpl.updateErrorStatus(objEntity.getMrefno(),Constants.MissingLeadError);
									continue;
								}
						...
						for (TradeAndNeighbourRefCheckEntity objEntity : listEntity) {
							...
							if(objEntity.getMleadsid()==null||("").equals(objEntity.getMleadsid())||(Constants.NULL).equals(objEntity.getMleadsid().trim())) {
								...
								if(customerLoanEntity!=null&&customerLoanEntity.getMleadsid()!=null&&!("").equals(customerLoanEntity.getMleadsid())) {
									...
									tradeAndNeighbourRefCheckServiceImpl.updateErrorStatus(objEntity.getMrefno(),Constants.MissingLeadError);
									continue;
								}
						...
						for (DeviationFormEntity objEntity : listEntity) {
							...
							if(objEntity.getMleadsid()==null||("").equals(objEntity.getMleadsid())||(Constants.NULL).equals(objEntity.getMleadsid().trim())  ) {
								...
								if(customerLoanEntity!=null&&customerLoanEntity.getMleadsid()!=null&&!("").equals(customerLoanEntity.getMleadsid())) {
									...
									deviationFormServiceImpl.updateErrorStatus(objEntity.getMrefno(),Constants.MissingLeadError);
									continue;
								}
				...
				public void synchronizeCustomerLoanCashFlow() {
					...
					if (listEntity != null) {
						...
						for (CustomerLoanCashFlowEntity objEntity : listEntity) {
							i++;
							leadIdNotPresent = false;
							System.out.println("I ki values = obj " + i);
							if(objEntity.getMleadsid()==null||("").equals(objEntity.getMleadsid())||("null").equals(objEntity.getMleadsid().trim())) {
								System.out.println("Finding for mrefno"+ objEntity.getMloanmrefno());
								leadIdNotPresent = true;
								CustomerLoanEntity customerLoanEntity =customerLoanRepository.findByMrefNo(objEntity.getMloanmrefno());
								if(customerLoanEntity!=null&&customerLoanEntity.getMleadsid()!=null&&!("").equals(customerLoanEntity.getMleadsid())
										&&!("null").equals(customerLoanEntity.getMleadsid().trim())
										) {
									objEntity.setMleadsid(customerLoanEntity.getMleadsid());	
								}
									...
									cashFlowServiceImpl.updateMerrorMessage(9,objEntity.getMrefno(),Constants.MissingLeadError);
									continue;
								}
							...
							try {
								...
								System.out.println("mrefno is -- " +objEntity.getMrefno());
								if (bean != null && bean.getStatus() == 0) {
									...
									System.out.println("Can not be updated  Error recieved -- " + bean.getError());
								}
							}catch(Exception e) {
						...
						for (CustomerLoanCPVBusinessRecordEntity objEntity : listEntity) {
							...
							if(objEntity.getMleadsid()==null||("").equals(objEntity.getMleadsid())||(Constants.NULL).equals(objEntity.getMleadsid().trim())) {
								...
								if(customerLoanEntity!=null&&customerLoanEntity.getMleadsid()!=null&&!("").equals(customerLoanEntity.getMleadsid())) {
									...
									cpvBusinessRecordServiceImpl.updateErrorStatus(objEntity.getMrefno(),Constants.MissingLeadError);
									continue;
								}
		...
		/*				for (int guaranterLoanMapHolder : listEntity) {
						...
						for (KycMasterEntity objEntity : listEntity) {
							...
							if(objEntity.getMleadsid()==null||("").equals(objEntity.getMleadsid())||(Constants.NULL).equals(objEntity.getMleadsid().trim())) {
								...
								if(customerLoanEntity!=null&&customerLoanEntity.getMleadsid()!=null&&!("").equals(customerLoanEntity.getMleadsid())) {
									...
									kycMasterServiceImpl.updateErrorStatus(objEntity.getMrefno(),Constants.MissingLeadError);
									continue;
								}
				...
				public void syncronizeIntrenalBankTransfer() {
				
					
					List<InternalBankTransferEntity> listEntity = internalBankTransferServiceImpl.getListNotSynced();
					System.out.println("internal Bank Transfer List Size hai " + listEntity.size());
					if (listEntity != null && listEntity.size() > 0) {
						
						for(InternalBankTransferEntity internalBankTransferBean  : listEntity) {
							
							//try {
								
								OmniSoapResultBean retBean = 	internalBankTransferServiceImpl.omniSoapServices(internalBankTransferBean);
								
								if(retBean==null) {
									
									
									internalBankTransferServiceImpl.updateResponseFromOmni("No Response",2,0,internalBankTransferBean.getMrefno());	
								}else if(retBean.getStatus()==1) {
									
									internalBankTransferServiceImpl.updateResponseFromOmni(retBean.getError(),1,0,internalBankTransferBean.getMrefno());
								}else if(retBean.getStatus()==0) {
									
									if(internalBankTransferBean.getMretry()<3) {
										System.out.println("updating with less  retry "+ internalBankTransferBean.getMretry());
										internalBankTransferServiceImpl.updateResponseFromOmni(retBean.getError(),0,(internalBankTransferBean.getMretry()+1),internalBankTransferBean.getMrefno());	
									}else {
										internalBankTransferServiceImpl.updateResponseFromOmni(retBean.getError(),2,0,internalBankTransferBean.getMrefno());
									}
...
//								
//								
//								internalBankTransferServiceImpl.updateResponseFromOmni("Something wrong",2,0,internalBankTransferBean.getMrefno());	
//								
//								
//							}
						}
