...
public class TDClosureServiceImpl implements TDClosureServices {
	...
	public TMsgInParam prepareRequestToOmni(BaseEntity bean) {
		...
		inParam.setField5(String.valueOf(tdClosureHolder.getMcreatedby()));//createdby
	...
	public void omniRequestCloseTD(TDClosureHolder bean) {
		...
		try 
		{
			...
			if(outparam!=null && outparam.getField1()!=null&&!("").equals(outparam.getField1().trim())){
				bean.setMerrormessage(outparam.getField1());
				bean.setMstatus(1);
				
			}
			else if(outparam!=null&&outparam.getField203()!=null) {
