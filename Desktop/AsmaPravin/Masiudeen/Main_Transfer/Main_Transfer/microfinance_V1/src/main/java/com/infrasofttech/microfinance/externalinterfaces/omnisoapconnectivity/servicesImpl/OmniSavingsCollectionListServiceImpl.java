	...
	public TMsgInParam prepareRequestToOmni(BaseEntity bean) {
		...
		for (int inparamsCount = 0; inparamsCount < this.beanList.size(); inparamsCount++) {
			...
			// changing prcdacctid
			  ...
			  String mprdcd = this.beanList.get(inparamsCount).getMprdacctid().substring(0,8).trim();
			...
			temp.append(this.beanList.get(inparamsCount).getMrefno()).append("~")
					...
					.append(Utility.removeZero(mcustno)+"/"+Utility.removeZero(mprdcd) +"/"+Utility.removeZero(mprcdacctid)).append("~")
