...
public interface ProspectCreationRepository extends JpaRepository<ProspectCreationEntity, Long> { 
	...
	@Query(value="SELECT * FROM  md970555 WITH (NOLOCK) where mcreatedby  =?1 AND mlastupdatedt >=?2 ",nativeQuery = true)
	List<ProspectCreationEntity> getProspectFromCreatedBy(String userName, LocalDateTime endDate);
	
	
	@Query(value="SELECT * FROM  md970555  WITH (NOLOCK) where mcreatedby  =?1  AND mProspectstatus= ?2 AND mlastupdatedt >=?3 ",nativeQuery = true)
	List<ProspectCreationEntity> getprspctfrmcrtdByAndprspctstatus(String userName, int prospectStatus, LocalDateTime endDate );
	
	@Query(value="SELECT * FROM  md970555 WITH (NOLOCK) where mcreatedby  =?1 AND  (mlastupdatedt > ?2 OR mlastsynsdate > ?2  ) AND mlastupdatedt >=?3 ",nativeQuery = true)
	List<ProspectCreationEntity> getProspectFromCreatedByAndLastSyncedDate(String userName, LocalDateTime lastSyncedDate , LocalDateTime endDate );
	
	
	@Query(value="SELECT * FROM  md970555 WITH (NOLOCK) where mcreatedby  =?1  AND mProspectstatus= ?2  AND  (mlastupdatedt > ?3 OR mlastsynsdate > ?3 ) AND mlastupdatedt >=?4   ",nativeQuery = true)
	List<ProspectCreationEntity> getprspctfrmcrtdByAndprspctstatusAndLastSyncedDate(String userName, int prospectStatus, LocalDateTime lastSyncedDate , LocalDateTime endDate);
