...
public class OmniTDReceiptsServiceImpl implements OmniTDReceiptsService {
	...
	public OmniSoapResultBean omniSoapServices(BaseEntity bean) {
		...
		try {
			...
			if(outparam!=null && outparam.getField1() !=null) {
				...
				retBean.setMbatchcd(outparam.getField4());
				if(outparam!=null ) {
					if(outparam.getField5() !=null) {
						retBean.setMsetno(Integer.parseInt(outparam.getField5().trim()));
					}
					if(outparam.getField6() !=null) {
						retBean.setMscrollno(Integer.parseInt(outparam.getField6().trim()));
					}
	...
	public TMsgInParam prepareRequestToOmni(BaseEntity bean) {
		...
		inParam.setField12(String.valueOf(tDReceiptsEntity.getMcreatedby()));
