...
public class CentersFoundationServiceImpl implements CentersFoundationService{
	...
	@Override
		 ...
		 */
			...
			return new ResponseEntity<Object>(centerFoundationRepo.saveAll(centerList),new HttpHeaders(),HttpStatus.CREATED);
		/*} catch (Exception e) {
	...
	public ResponseEntity<?> getDataUserByMcenterNameAndMrefno(String mcentername, int mrefno) {
		try {
			List<CentersFoundationEntity> centerList=centerFoundationRepo.findByMcenternameAndMrefno(mcentername, mrefno);
			if(centerList.isEmpty()) 
				return ResponseEntity.notFound().build();
			return new ResponseEntity<List<CentersFoundationEntity>>(centerList,new HttpHeaders(),HttpStatus.OK);
		}catch(Exception e) {
	...
	public ResponseEntity<?> getDataByMcreatedByAndMlbrCode(String mCreatedBy, int mLbrcode) {
		try {
			List<CentersFoundationEntity> centerList=centerFoundationRepo.findByMcreatedByAndMlbrCode(mCreatedBy,mLbrcode);
			if(centerList.isEmpty()) 
				return ResponseEntity.notFound().build();
			return new ResponseEntity<List<CentersFoundationEntity>>(centerList,new HttpHeaders(),HttpStatus.OK);
		}catch(Exception e) {
	...
	public int updateCentersErrorfromOmni(int mCenterRefNo, String errorFromOmni) {
		...
		try {
			...
			isCentersListUpdated =  centerFoundationRepo.updateCentersErrorfromOmni(mCenterRefNo, errorFromOmni, updatedDt);
	...
	@Override
	public int updateCenterLocation(CentersFoundationEntity centerEntity) {
		// TODO Auto-generated method stub
		
		
		return centerFoundationRepo.updateCenterLocationFrmMrefno(centerEntity.getMgeolatd(),
				centerEntity.getMgeologd(),centerEntity.getMrefno());
	}
