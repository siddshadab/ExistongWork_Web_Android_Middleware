...
public interface CustomerLoanRepository extends JpaRepository<CustomerLoanEntity,Long> {
	...
	*/
	...
	@Modifying
    @Query(value = "UPDATE md045011 SET mleadsid = ?2 ,mlastupdatedt =?3, missynctocoresys=1 WHERE mrefno = ?1  ",nativeQuery = true)
	...
	@Query(value="SELECT * FROM  md045011  WITH (NOLOCK) where (mcreatedby=?1 OR (mrouteto=?1 AND missynctocoresys = 1 AND mleadstatus!=7)) AND mlastupdatedt > ?2",nativeQuery = true)
	List<CustomerLoanEntity> findByCreatedbyAndDateTime(String mcreatedby,LocalDateTime lastsynsdate);

	
	@Query(value="SELECT  * FROM  md045011  WITH (NOLOCK) WHERE (mcreatedby=?1  OR  (mrouteto=?1 AND missynctocoresys = 1)) AND mlastupdatedt > ?2  order BY mlastupdatedt  desc",nativeQuery = true)
	List<CustomerLoanEntity> findByCreatedbyANDMinDate(String mcreatedby,LocalDateTime lastUpdtDate);
	
	
	List<CustomerLoanEntity> findByMcreatedbyAndMrouteto(String mcreatedby , String mrouteto);
	
	@Modifying
    @Query(value = "UPDATE md045011 SET mcustno = ?2 ,mlastupdatedt =?3 WHERE mcustmrefno = ?1  ",nativeQuery = true)
	...
	@Modifying
    @Query(value = "UPDATE md045011 SET mleadsid = ?2 ,mlastupdatedt =?3, missynctocoresys=1, mprdacctid = ?3  WHERE mrefno = ?1  ",nativeQuery = true)
    int updateCustomerLoanLeadIdAndMprdacctId( int mLoanRefNo, String mleadsid,LocalDateTime updatedDt,String mprdacctid);
	
	@Query(value="SELECT TOP 10 * FROM  md045011 WITH (NOLOCK) WHERE  trefno = ?1 AND mcreateddt = ?2 AND  mcreatedBy = ?3  ",nativeQuery = true)
	List<CustomerLoanEntity> findByTrefnoandCreateddt(int trefno, LocalDateTime createddt, String mcreatedBy);
	
	
	
	
	@Modifying
    @Query(value = "UPDATE md045011  SET misdisbursed = 1 WHERE mleadsid = ?1  ",nativeQuery = true)
    int updateIsDisbursed( String mleadsId);
