...
public class CustomerServiceImpl implements CustomerService {
	...
	public ResponseEntity<List<CustomerEntity>> addCustomerListByHolder(List<CustomerHolderBean> customerList) {
		try { 		
			...
			List<CustomerFingerPrintEntity> addFingerPrintList;
			...
			if(null != customerList) {
				for(CustomerHolderBean bean :customerList) {
					...
					addFingerPrintList=new ArrayList<CustomerFingerPrintEntity>();

					if(bean.getMrefno()>0) {
						entityBean.setMrefno(bean.getMrefno());
						
					//Changes done for if  custstatus not updated in tab 
						...
						//Changes done for if  custstatus not updated in tab
						entityBean.setMcuststatus(custEntity.getMcuststatus());
						...
						if(bean.getImageMaster()!=null&&!bean.getImageMaster().isEmpty()) {
						...
						if(bean.getCustomerFingerPrintList()!=null&&!bean.getCustomerFingerPrintList().isEmpty()) {
							fingerPrintRepo.deleteExistingRecords(bean.getMrefno());
							System.out.println("Deleting Finger print records");
						}
					...
					entityBean.setMismodify(bean.getMismodify());
					entityBean.setMissyncfromcoresys(bean.getMissyncfromcoresys());
					...
					for(CustomerFingerPrintEntity customerFingerPrint :  bean.getCustomerFingerPrintList() ) {
						customerFingerPrint.setCustomerFingerPrintEntity(entityBean);;
						addFingerPrintList.add(customerFingerPrint);						
					}
					entityBean.setCustomerFingerPrintList(addFingerPrintList);
...
//			    }
				...
				if(customerListEntity != null)
				{
					...
					System.out.println("adding Image Entity");
					  ...
					  List<CustomerFingerPrintEntity> custFingPrintEnt =
							  fingerPrintRepo.findByMrefno(customerListEntity.getMrefno());
							  
							  customerListEntity.setCustomerFingerPrintList(custFingPrintEnt);
							  System.out.println("Added Image Entity"); 
	...
	public ResponseEntity<?> findByCreatedByAndLastSyncedTime(CustomerEntity customerEntity) {		
		try {
//			String getImages = systemParamServiceImpl.getCodeValue("GETIMAGES");
//			String getFingerPrint = systemParamServiceImpl.getCodeValue("GETFINGERPRINT");
			...
			if(customerEntity.getMlastsynsdate()==null ) {
				...
				customerList=customerRepository.findByCreatedbyAndNotSynced(customerEntity.getMcreatedby());
			}
...
//			    }
				...
				if(customerListEntity != null)
				{
					...
					if(Constants.getImages==null||Constants.getImages.trim().equalsIgnoreCase("1")) {
						List<ImageMasterEntity> custImgTempEnt =
								  imageMasterRepository.findByMrefno(customerListEntity.getMrefno());
								  if(custImgTempEnt!=null&&custImgTempEnt.size()>0) {
						...
						//System.out.println("Koi images nai uth rai");
						customerListEntity.setImageMaster(new ArrayList<ImageMasterEntity>() );
					}
					  ...
					  System.out.println("Added Business expense entity "+ customerListEntity.getMrefno());
					  ContactPointVerificationEntity custCpvTempEnt = (ContactPointVerificationEntity)
							  contactPointVerificationRepository.findByMrefno(customerListEntity.getMrefno());
					  customerListEntity.setContactPointVerification(custCpvTempEnt);
					  ...
					  if(Constants.getFingerPrint==null || Constants.getFingerPrint.trim().equalsIgnoreCase("1")) {
							List<CustomerFingerPrintEntity> custFingTempEnt =
									  fingerPrintRepo.findByMrefno(customerListEntity.getMrefno());
									  if(custFingTempEnt !=null&&custFingTempEnt.size()>0) {
										  
										  customerListEntity.setCustomerFingerPrintList(custFingTempEnt);
										  System.out.println("Added Image Entity"); 
									  }
						}else {
							
							System.out.println("Koi images nai uth rai");
							  customerListEntity.setCustomerFingerPrintList(new ArrayList<CustomerFingerPrintEntity>() );
						}
			...
			System.out.println("Manipulate Cpmpleted");
...
//			String getFingerPrint = systemParamServiceImpl.getCodeValue("GETFINGERPRINT");
			...
			if(customerEntity != null)
			{
				...
				if(Constants.getImages!=null&&Constants.getImages.trim().equalsIgnoreCase("1")) {
					...
					//System.out.println("Koi images nai uth rai");
					customerEntity.setImageMaster(new ArrayList<ImageMasterEntity>() );
				}
				  ...
				  if(Constants.getFingerPrint==null || Constants.getFingerPrint.trim().equalsIgnoreCase("1")) {
						...
						List<CustomerFingerPrintEntity> custFingTempEnt =
								  fingerPrintRepo.findByMrefno(customerEntity.getMrefno());
								 
									  
									  customerEntity.setCustomerFingerPrintList(custFingTempEnt);
									
							
					}else {
						
						System.out.println("Koi images nai uth rai");
						customerEntity.setCustomerFingerPrintList(new ArrayList<CustomerFingerPrintEntity>() );
					}
...
//		
		...
		entity.setImageMaster(custImgTempEnt);
		  ...
		  List<CustomerFingerPrintEntity> custFingTempEnt =
				  fingerPrintRepo.findByMrefno(entity.getMrefno());
				entity.setCustomerFingerPrintList(custFingTempEnt);
				  System.out.println("Added Finger Print Entity"); 
...
//		}	}
	...
	public ResponseEntity<?> getFingerPrintByCustNo(int custNo) {
		try {
			List<CustomerFingerPrintEntity> fingerPrintList = fingerPrintRepo.getByFingerPrint(custNo);
			if(fingerPrintList.isEmpty()) 
				return ResponseEntity.notFound().build();
	...
	public int updateCustStatusInCustomerDetails(int custno,int custStatus) {
		...
		@Transactional
		@Override
		public ResponseEntity<CustomerModificationAccessHolder> updateCustomerModificationAccess(
				CustomerModificationAccessHolder accessHolder) {
			// TODO Auto-generated method stub
			try {	
			LocalDateTime  updateDtTime = LocalDateTime.now();
			int i = customerRepository.updateCustModRight(accessHolder.getMcustno(), updateDtTime,accessHolder.getMlbrcode() );
			if(i==1) {
				accessHolder.setMissynctocoresys( 1);
				accessHolder.setMerrormessage("Update Done Successfully");
				
				
			}else {
				accessHolder.setMissynctocoresys(2);
				accessHolder.setMerrormessage("No Customer Found");
				
			}
			System.out.println(accessHolder);
			return new ResponseEntity<CustomerModificationAccessHolder>(accessHolder,new HttpHeaders(),HttpStatus.OK);
		...
		@Transactional(readOnly=true)
	public ResponseEntity<CustomerEntity> getCustomerDetails(CustomerDedupHolder customerDedupHolder){
		try {
			
			
			CustomerEntity customerEntity=null;
			if(customerDedupHolder.getMcustno()!=0) {
			 customerEntity = customerRepository.findByMcustNo(customerDedupHolder.getMcustno());
			}else if(customerDedupHolder.getMnationalid()!=null&&!customerDedupHolder.getMnationalid().equals("")) {
				customerEntity = customerRepository.findByOnlyMnationalId(customerDedupHolder.getMnationalid());
			}
			if(customerEntity!=null) {
				...
				List<ImageMasterEntity> custImgTempEnt =
						  imageMasterRepository.findByMrefno(customerEntity.getMrefno());
				
						  
				customerEntity.setImageMaster(custImgTempEnt);
				System.out.println(customerEntity.getImageMaster().size());
		...
		public void updateSyncFromCore(List<Integer> mrefNoList){
			try {
				System.out.println("Yhan tak ayaa");
				customerRepository.updateCustSyncFromServer(mrefNoList);
				System.out.println("Yhan tak bhi ayaa");
		...
		@Transactional		
		public ResponseEntity<?> findByCenterIdForLocations(CustomerEntity customerEntity) {
			
		try {
				List<CustomerLocationHolder> customerList=customerRepository.findByMcenterid(customerEntity.getMcenterid());
	
			//List<CustomerLocationHolder> customerList=customerRepository.findByCenterIdSpecific2(customerEntity.getMcenterid());
			if(customerList.isEmpty()) 
				return ResponseEntity.notFound().build();
			return new ResponseEntity<List<CustomerLocationHolder>>(customerList,new HttpHeaders(),HttpStatus.OK);
	}catch(Exception e) {
			System.out.println("No Record Found. wasasas : "+e.getStackTrace());
			//logger.error("No Record Found."+e.getMessage());
			return new ResponseEntity<>(HttpStatus.UNPROCESSABLE_ENTITY);
