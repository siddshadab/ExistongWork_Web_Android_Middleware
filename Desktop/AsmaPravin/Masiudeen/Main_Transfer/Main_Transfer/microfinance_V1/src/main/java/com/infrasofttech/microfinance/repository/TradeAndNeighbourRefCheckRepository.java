...
public interface TradeAndNeighbourRefCheckRepository extends JpaRepository<TradeAndNeighbourRefCheckEntity, Long> {
	...
	@Modifying
    @Query(value="SELECT top 50 * FROM  md045803 where missynctocoresys =?1 AND mloanmrefno != 0 ",nativeQuery = true)
	...
	@Modifying
	@Query(value = "UPDATE md045803 SET mleadsid= ?1 ,missynctocoresys = 0, mlastupdatedt = ?4 , merrormessage =''  WHERE mloanmrefno = ?2 AND mloantrefno = ?3  ",nativeQuery = true)
	...
	List<TradeAndNeighbourRefCheckEntity> findByCreatedby(String mcreatedby);
	  ...
	  @Modifying
	  @Query(value = "UPDATE md045803 SET merrormessage= ?1 , missynctocoresys = 2 ,mlastupdatedt = ?4 WHERE mloanmrefno = ?2 AND mloantrefno = ?3  ",nativeQuery = true)
