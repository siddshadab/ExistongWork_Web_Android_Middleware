...
public class OmniGroupMasterServiceImpl implements OmniSavingsListService {
	...
	public TMsgInParam prepareRequestToOmni(BaseEntity bean) {
		...
		if (groupsFoundationEntity.getMgroupid() == 0)
			inParam.setField1("0~C~2"+Constants.TILDA+groupsFoundationEntity.getTrefno()+Constants.HIFEN+groupsFoundationEntity.getMrefno());
		else
			inParam.setField1(String.valueOf(groupsFoundationEntity.getMgroupid()) + "~N~2"+Constants.TILDA+groupsFoundationEntity.getTrefno()+Constants.HIFEN+groupsFoundationEntity.getMrefno());
