...
public class CustomerLoanServiceImpl implements CustomerLoanService {
	...
	public ResponseEntity<?> addList(List<CustomerLoanEntity> customerLoan) {
		try {
			...
			for(CustomerLoanEntity bean: customerLoan) {
				bean.setMcreateddt(bean.getMcreateddt().withNano(0));
				System.out.println("Trying to add for "+ bean.getMrefno());
				if( bean.getMrefno()!=0 &&  (bean.getMleadsid() == null || "".equals(bean.getMleadsid()))  ) {
					...
					if(bean.getMrefno()==0) {
						System.out.println(bean.getTrefno() +" "+bean.getMcreateddt() +" "+ bean.getMcreatedby() );
						List<CustomerLoanEntity> customerLoanList = new ArrayList<CustomerLoanEntity>();
						customerLoanList =  loanRepository.findByTrefnoandCreateddt(bean.getTrefno(),
								bean.getMcreateddt(),bean.getMcreatedby());
						System.out.println("Returned List is "+ customerLoanList);
						if(customerLoanList!=null&&!customerLoanList.isEmpty()) {
					...
					System.out.println("xxxxxPhle se nai hai ");
					customerLoanAdd.add(bean);
			...
			return new ResponseEntity<Object>(loanRepository.saveAll(customerLoanAdd),new HttpHeaders(),HttpStatus.CREATED);
		} catch (Exception e) {
	...
	public ResponseEntity<?> findByCreatedByOrMrouteToAndLastSyncedTime(CustomerLoanEntity customerLoanEntity) {			
		try { 
			...
			LocalDateTime dt = LocalDateTime.now().minusDays(Constants.leadValidityDays);
			...
			if(customerLoanEntity.getMlastsynsdate()==null  ) {
				...
				customerLoanEntityList = loanRepository.findByCreatedbyANDMinDate(customerLoanEntity.getMcreatedby(),dt);
				
			}else if(customerLoanEntity.getMlastsynsdate()!=null ) {
				
				if(dt.isAfter(customerLoanEntity.getMlastsynsdate())) {
					customerLoanEntity.setMlastsynsdate(dt);
				}
			...
			for(CustomerLoanEntity cutLoanBean:customerLoanList) {
				...
				if(/*!(customerLoanEntity.getMcreatedby()!=null &&
					...
					cutomerLoanTillGrtHolder.setMisdisbursed(cutLoanBean.getMisdisbursed());
	...
	@Override
	public int updateCustomerLoanLeadIdAndPrdacctIdOfCore(String loanLeadId, String prdacctid,int mrefno) {
		int isCustomerUpdated = 0 ;
		try {
			LocalDateTime dateTime = LocalDateTime.now();
			isCustomerUpdated = loanRepository.updateCustomerLoanLeadIdAndMprdacctId(mrefno,loanLeadId,dateTime,prdacctid);
	...
	public int updateIsDisbursed(String loanLeadId) {
		try {
			...
			loanRepository.updateIsDisbursed(loanLeadId);
		}catch(Exception e) {
