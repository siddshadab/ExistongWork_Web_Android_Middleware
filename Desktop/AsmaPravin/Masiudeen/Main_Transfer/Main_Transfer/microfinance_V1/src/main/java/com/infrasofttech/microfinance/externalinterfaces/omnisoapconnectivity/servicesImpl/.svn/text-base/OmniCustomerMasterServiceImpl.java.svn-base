	...
	public OmniSoapResultBean omniSoapServices(BaseEntity bean) {
		...
		try {
			...
			if(customerEntityBean.getMcustno()>0) {
				outparam = omniServiceSoapProxy.invoke(Constants.Field1, "2", 910906, inParams,true);
			}else {
				outparam = omniServiceSoapProxy.invoke(Constants.Field1, "1", 910906, inParams,true);	
			}
		...
		try {
			...
			if(customerEntityBean.getCustomerPPIDetailsEntity()!=null 
				...
				for (CustomerPPIDetailsEntity customerPPIDetailsEntity : ppiList) {
					temp="";	        		
					temp=temp+customerPPIDetailsEntity.getMquestiontype()+Constants.TILDA+//0,QnsType
