...
@Service
public class OmniDisbursedListServiceImpl implements OmniDisbursedListService{
	...
	public List<DisbursedListEntity> omniSoapServicesDisbursedList(List<DisbursedListEntity> beanList,boolean isOnline) {
		...
		try {
			...
			System.out.println("Datad of out param "+outparam);
			System.out.println("here starts");
			System.out.println(outparam.getField1());
			System.out.println(outparam.getField2());
			System.out.println(outparam.getField101());
			...
			System.out.println(outparam.getField201());
			System.out.println(outparam.getField204());
			System.out.println(outparam.getField203() + " 203 hai yeh Disbursment Time ");
			System.out.println(outparam.getField202());
			System.out.println("here ends");
			
//			if(outparam.getField1()==null||outparam.getField1().equals("")) {
//				
//				return new ArrayList<DisbursedListEntity>();
//			}
			//System.out.println(" Sai chal rha hai ");
	...
	public TMsgInParam prepareRequestToOmni(BaseEntity bean) {
		...
		for(int inparamsCount =0;inparamsCount<this.beanList.size();inparamsCount++) {
			...
			temp.append(mode).append("~").append(String.valueOf(sdCashFlag)).append("~");
	...
	public List<DisbursedListEntity> prepareResponseRecievedFromOmniForCollection(TMsgOutParam bean) {
		// TODO Auto-generated method stub
		//System.out.println(" Response taiyaar ho rha hai");
		...
		//System.out.println(" bean response here is "+bean.toString());
		      ...
		      f1=null;
		        	for(int arrOfStringSingles =0;arrOfStringSingles<=arrOfStr.length;arrOfStringSingles++) {
		        		...
		        		try {
				        	...
				        	try {
				        		if(disbBean.getMissynctocoresys()==1) {
				        			customerLoanServiceImpl.updateIsDisbursed(disbBean.getMleadsid());
				        		}
		        	...
		        	//disbRepo.updateStatus(disbBean.getMleadsid(),disbBean.getMissynctocoresys(),disbBean.getMerrormessage(),LocalDateTime.now());
		        	
		        	
		        	for(int i = 0;i<this.beanList.size();i++) {
		        		if(this.beanList.get(i).getMleadsid().equals(disbBean.getMleadsid())) {
		        			this.beanList.get(i).setMissynctocoresys(disbBean.getMissynctocoresys());
		        			this.beanList.get(i).setMerrormessage(disbBean.getMerrormessage());
		        			this.beanList.get(i).setMentrydate(disbBean.getMentrydate());
		        			this.beanList.get(i).setMbatchcd(disbBean.getMbatchcd());
		        			this.beanList.get(i).setMsetno(disbBean.getMsetno());
		        			this.beanList.get(i).setMcurcd(disbBean.getMcurcd());
			...
			// TODO Auto-generated catch block
				//System.out.println("Kuch Fta hau");
				try {
					
					if(bean!=null && !("0").equals(bean.getField201().trim())) {
						if(this.isOnline==true) {
							//System.out.println("is Online");
				        			this.beanList.get(0).setMissynctocoresys(2);
				        			this.beanList.get(0).setMerrormessage(bean.getField203());
					...
					}else if(bean!=null && ("0").equals(bean.getField201().trim())) {
						if(this.isOnline==true) {
							//System.out.println("is Online");
				        			this.beanList.get(0).setMissynctocoresys(1);
				        			this.beanList.get(0).setMerrormessage(bean.getField203());
					...
					System.out.println("Dusre exception k andar aaya");
		...
		// etc.
			...
			//System.out.println("Bean Lisy is "+ this .beanList.get(0).getMissynctocoresys());
